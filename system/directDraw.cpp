...
BOOL InitDirectDraw(void)
{
	...
	if ((hResult = DirectDrawCreate(NULL, &lpDraw->lpDD, NULL)) != DD_OK) {
		...
		if ((hResult = DirectDrawCreate((GUID *)DDCREATE_EMULATIONONLY, &lpDraw->lpDD, NULL)) != DD_OK) {
			MessageBoxNew(hWnd, "DirectDrawCreate Error", "확인", MB_OK | MB_ICONSTOP);
			hErrorLogFile = CreateFile("ErrorLog.txt", GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	if ((hResult = lpDraw->lpDD->QueryInterface(IID_IDirectDraw2, (LPVOID *)&lpDraw->lpDD2)) != DD_OK) {
		MessageBoxNew(hWnd, "QueryInterface Error", "확인", MB_OK | MB_ICONSTOP);
		hErrorLogFile = CreateFile("ErrorLog.txt", GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
#undef _BACK_VERSION
	...
	if (WindowMode) {
		...
		if (lpDraw->lpDD2->SetCooperativeLevel(hWnd, DDSCL_NORMAL) != DD_OK) {
			MessageBoxNew(hWnd, "SetCooperativeLevel Error", "확인", MB_OK | MB_ICONSTOP);
			return FALSE;
		}
		...
		if ((hResult = lpDraw->lpDD2->CreateSurface(&lpDraw->ddsd, &lpDraw->lpFRONTBUFFER, NULL)) != DD_OK) {
			MessageBoxNew(hWnd, "메인화면처리실패。", "확인", MB_OK | MB_ICONSTOP);
			hErrorLogFile = CreateFile("ErrorLog.txt", GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		...
		if (lpDraw->lpDD2->CreateClipper(0, &lpDraw->lpCLIPPER, NULL) != DD_OK) {
			MessageBoxNew(hWnd, "clipper처리실패。", "확인", MB_OK | MB_ICONSTOP);
			return FALSE;
		}
		...
		if ((hResult = lpDraw->lpDD2->CreateSurface(&lpDraw->ddsd, &lpDraw->lpBACKBUFFER, NULL)) != DD_OK) {
			MessageBoxNew(hWnd, "레지스트리 처리 실패", "확인", MB_OK | MB_ICONSTOP);
			hErrorLogFile = CreateFile("ErrorLog.txt", GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		...
		if (g_bUseAlpha) {
			...
			if ((hResult = lpDraw->lpDD2->CreateSurface(&lpDraw->ddsd, &lpDraw->lpBACKBUFFERSYS, NULL)) != DD_OK) {
				MessageBoxNew(hWnd, "暂存区处理失败(sys)", "확인", MB_OK | MB_ICONSTOP);
				hErrorLogFile = CreateFile("ErrorLog.txt", GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
				sprintf_s(szErrMsg, "Create backbuffer error:error result (%x)", hResult);
				WriteFile(hErrorLogFile, szErrMsg, sizeof(szErrMsg), &dwWriteByte, NULL);
				CloseHandle(hErrorLogFile);
...
#define _BACK_VERSION
#endif
	{
		...
		if ((hResult = lpDraw->lpDD2->CreateSurface(&lpDraw->ddsd, &lpDraw->lpFRONTBUFFER, NULL)) != DD_OK) {
			MessageBoxNew(hWnd, "메인 화면 처리 실패 2。", "확인", MB_OK | MB_ICONSTOP);
			hErrorLogFile = CreateFile("ErrorLog.txt", GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		...
		if (g_bUseAlpha) {
			...
			lpDraw->ddsd.ddsCaps.dwCaps = DDSCAPS_OFFSCREENPLAIN | DDSCAPS_SYSTEMMEMORY;
			if ((hResult = lpDraw->lpDD2->CreateSurface(&lpDraw->ddsd, &lpDraw->lpBACKBUFFERSYS, NULL)) != DD_OK) {
				MessageBoxNew(hWnd, "暂存区处理失败二(sys)", "확인", MB_OK | MB_ICONSTOP);
				hErrorLogFile = CreateFile("ErrorLog.txt", GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
				sprintf_s(szErrMsg, "Create backbuffer error:error result (%x)", hResult);
				WriteFile(hErrorLogFile, szErrMsg, sizeof(szErrMsg), &dwWriteByte, NULL);
				CloseHandle(hErrorLogFile);
...
#ifdef _STONDEBUG_
		MessageBoxNew(hWnd, "建立VideoRam BattleSurface失败！", "확인", MB_OK | MB_ICONSTOP);
#endif
		if ((lpBattleSurface = CreateSurface(DEF_APPSIZEX, DEF_APPSIZEY, DEF_COLORKEY, DDSCAPS_SYSTEMMEMORY)) == NULL) {
			MessageBoxNew(hWnd, "BattleSurface 구성 실패！", "확인", MB_OK | MB_ICONSTOP);
			return FALSE;
		}
	}
#ifdef _READ16BITBMP
	if (g_bUseAlpha) {
		if ((lpBattleSurfaceSys = CreateSurface(DEF_APPSIZEX, DEF_APPSIZEY, DEF_COLORKEY, DDSCAPS_SYSTEMMEMORY)) == NULL) {
			MessageBoxNew(hWnd, "建立BattleSurface(sys)失败！", "확인", MB_OK | MB_ICONSTOP);
			return FALSE;
		}
	}
#endif
...
BOOL InitPalette(void)
{
	...
	//新系统色盘//
	{ 0xde, 0x00, 0x00, PC_NOCOLLAPSE | PC_RESERVED },//10
	{ 0xff, 0x5f, 0x00, PC_NOCOLLAPSE | PC_RESERVED },//11
	{ 0xff, 0xff, 0xa0, PC_NOCOLLAPSE | PC_RESERVED },//12
	{ 0x00, 0x5f, 0xd2, PC_NOCOLLAPSE | PC_RESERVED },//13
	{ 0x50, 0xd2, 0xff, PC_NOCOLLAPSE | PC_RESERVED },//14
	{ 0x28, 0xe1, 0x28, PC_NOCOLLAPSE | PC_RESERVED },//15
	...
	if (PalState.flag == FALSE) {
		...
		if (fp == NULL) {
			MessageBoxNew(hWnd, "인덱스 파일 읽기", "Error", MB_OK | MB_ICONSTOP);
			return FALSE;
		}
		else {
			//可自由使用的调色盘设定// ??????????
	...
	if (lpDraw->lpPALETTE == NULL) {
		MessageBoxNew(hWnd, "팔레트 처리 실패", "Error", MB_OK | MB_ICONSTOP);
		return FALSE;
	}
	// WON REM 
#ifdef _HI_COLOR_16
	...
	if (displayBpp == 8) {
		if (lpDraw->lpFRONTBUFFER->SetPalette(lpDraw->lpPALETTE) != DD_OK) {
			MessageBoxNew(hWnd, "팔레트 처리 실패", "Error", MB_OK);
			MessageBoxNew(hWnd, "16비트 하이 컬러 또는 32비트 하이 컬러 프레임을 사용해주세요", "Error", MB_OK);
			return FALSE;
		}
	}
#else
...
#ifdef SWITCH_MODE
	if (lpDraw->lpFRONTBUFFER->SetPalette(lpDraw->lpPALETTE) != DD_OK) {
		if (MessageBoxNew(hWnd, "请使用２５６色的显示模示", "확인", MB_RETRYCANCEL | MB_ICONEXCLAMATION) == IDCANCEL)
			return FALSE;
		return FALSE;
	}
#else
...
void Flip(void)
{
	...
	//JL hook
	if (g_hideSA || !lpDraw->lpFRONTBUFFER){
		return;
	}
	...
	try{
		if (WindowMode == TRUE) {
			...
			//		lpDraw->lpDD2->WaitForVerticalBlank(DDWAITVB_BLOCKEND, NULL);  JL fix
...
LPBITMAPINFO LoadDirectDrawBitmap(char *pFile)
{
	try
	{
		...
		if ((lpBmpInfo = (LPBITMAPINFO)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, BmpFileHeader.bfSize)) == NULL) {
			MessageBoxNew(hWnd, "Heap的配置记忆体失败！", "확인", MB_OK | MB_ICONSTOP);
...
void DrawBitmapToSurface2(SURFACE_INFO *surface_info, LPDIRECTDRAWSURFACE lpSurfaceSys, int offsetX, int offsetY, int sizeX, int sizeY, LPBITMAPINFO pBmpInfo)
{
	...
	if (lpSurface->Unlock(NULL) != DD_OK) {
		//MessageBoxNew( hWnd, "Surface的Unlock失败！", "확인", MB_OK | MB_ICONSTOP );
		return;
	}
#ifdef _READ16BITBMP
...
void DrawBox(RECT *rect, unsigned char color, BOOL fill)
{
	...
	try
	{
		...
		if (lpDraw->lpBACKBUFFER->Lock(NULL, &ddsd, DDLOCK_WAIT, NULL) != DD_OK) {
			//MessageBoxNew( hWnd, "Surface的lock失败！", "확인", MB_OK | MB_ICONSTOP );
			return;
		}
#ifdef _READ16BITBMP
		if (g_bUseAlpha) {
...
void InitFont(int fontNo)
{
	...
	if (ResoMode == 1)
	{
		hFont = CreateFont(FONT_SIZE / 2, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, HANGEUL_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, FIXED_PITCH | FF_ROMAN, TEXT("굴림체"));
	}
	else
	{
		hFont = CreateFont(FONT_SIZE, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, HANGEUL_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, FIXED_PITCH | FF_ROMAN, TEXT("굴림체"));
	}
}

#ifdef _CHANNEL_MODIFY
char g_szChannelTitle[][13] = { "[普]", "[密]", "[队]", "[族]", "[职]", "[聊]"
...
void PutText(char fontPrio)
{
	...
	try
	{
		...
		if (lpDraw->lpBACKBUFFER == NULL) return;
...
void snapShot( void )
{
...
#ifdef  _STONDEBUG_
		MessageBox(hWnd,"前景缓冲区锁定失败！", "확인", MB_OK | MB_ICONSTOP );
#endif
		return;
	}
	/*
	...
	if(displayBpp == 16){
		...
		if(mem == NULL){
#ifdef  _STONDEBUG_
			MessageBox(hWnd,"记忆体配置失败失败！", "확인", MB_OK | MB_ICONSTOP );
#endif
			return;
		}
...
//	HFONT JL_FONT();
	...
	return CreateFont(  // 윈도우창 폰트
		...
		/* ?????	*/ 
		HANGUL_CHARSET/*CHINESEBIG5_CHARSET*/,// ????
