...
#define BRAN_SIZE_Y 16
#endif
extern int BRAN_SIZE_X;
extern int BRAN_SIZE_Y;
static int bran_cnt;
//static int pos_tbl[( ( 480 + DISPLACEMENT_Y ) /BRAN_SIZE_Y)*( ( 640+ DISPLACEMENT_X )/BRAN_SIZE_X)*2];
static int pos_tbl[((480 + 288) / 15)*((640 + 384) / 20) * 2];
...
BOOL DrawProduce( int no )
{
	...
	case PRODUCE_UP_DOWM_BRAKE:	// // ???
		ret = ProduceBrake( 5 );
		break;
	case PRODUCE_4WAY_OUT:		// ?????
		ret = Produce4Way( 0 );
		break;
	case PRODUCE_4WAY_IN:		// ????
		ret = Produce4Way( 1 );
		break;
...
//		y[ 9 ] = 410;
	...
	if( cnt3 < 8 ){
		// 
		for( i = 0 ; i < 8 ; i++ ){
			StockDispBuffer( ( int )x[ i ] + DISPLACEMENT_X / 2, ( int )y[ i ] + DISPLACEMENT_Y / 2, DISP_PRIO_CHAR, CG_TITLE_NAME_S + i, 0 );
		}
	}else if( cnt3 >= 8 && cnt3 < 31 ){
		if( cnt3 < 26 ){
			StockDispBuffer( ( int )x[ 0 ] + DISPLACEMENT_X / 2, ( int )y[ 0 ] + DISPLACEMENT_Y / 2, DISP_PRIO_CHAR, CG_TITLE_NAME_FLASH, 0 );
		}else{
			StockDispBuffer( ( int )x[ 0 ] + DISPLACEMENT_X / 2, ( int )y[ 0 ] + DISPLACEMENT_Y / 2, DISP_PRIO_CHAR, CG_TITLE_NAME_FLASH1 + cnt3 - 26 , 0 );
		}	
		cnt3++;
	}else
		StockDispBuffer( ( int )x[ 0 ]  +DISPLACEMENT_X / 2, ( int )y[ 0 ] + DISPLACEMENT_Y / 2, DISP_PRIO_CHAR, CG_TITLE_NAME, 0 );
}
