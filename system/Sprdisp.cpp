...
void PutBmp(void)
{
	...
	{
		if (lpDraw == 0) return;
		if (lpDraw->lpBACKBUFFER == 0)return;
...
int StockDispBuffer(int x, int y, UCHAR dispPrio, int bmpNo, BOOL hitFlag)
{
	...
	if (bmpNo > CG_INVISIBLE) {
		if (bmpNo > 200000 && bmpNo < 500000){
			BmpNo = bmpNo;
		}else{
			realGetNo(bmpNo, (U4 *)&BmpNo);
			if (BmpNo == 0)BmpNo = bmpNo;  //jl fix 直接显示动画号
		}
...
#ifdef _SHOW_bmpNo
	extern int g_showBmpNo;
	if (g_showBmpNo){
		if (bmpNo > 0 && dispPrio != DISP_PRIO_TILE){
			char buf[30];
			wsprintfA(buf, "%d", bmpNo);
			StockFontBuffer(pDispInfo->x, pDispInfo->y, FONT_PRIO_FRONT, 6, buf, 0);
		}
	}
...
int StockDispBuffer绝对位置(int x, int y, UCHAR dispPrio, int bmpNo, BOOL hitFlag)
{
	short dx, dy;
	int BmpNo;
...
#ifdef _CAX_LV1_STOP_AI
extern int AI_OtherSetting2[1];
