...
void AI_CheckSetting()
{
	...
	if (method < MAX_MAGIC && method >= 0) {//使用精灵
		if (magic[method].useFlag && (str = strstr(magic[method].memo, "내구력")) && strstr(str, "회복")) {//使用耐久力回复精灵
...
void AI_SetUpCureMethod(int which)
{
	...
	for (int i = method; i < MAX_MAGIC; i++) {
		if (magic[i].useFlag && (str = strstr(magic[i].memo, "내구력")) && strstr(str, "회복")) {
			AI_method[which] = i;
...
BOOL AI_SettingProc()
{
	static char Notice[][32] = { "설정저장", "설정취소",
...
#endif
#ifdef _AI_OTHER
			StockFontBuffer(x + 322, y + 173, FONT_PRIO_FRONT, FONT_PAL_WHITE, "기타기능", 0);
#endif
			char limit[8];
			int dy = 102;
#ifndef _ATTACK_AI
	...
	if (p_party[BattleMyNo]->hp <= 0 ||
		...
		if (AI_First) {//第一次
			...
			if (bFirstFlag == false) {
				...
				//	else
				//		StockChatBufferLine( "没有设定首次的精灵", FONT_PAL_WHITE);
#ifdef _AI_NEW_FIRST
			}
			else {
				int ret;
		...
		if (0 == done) {
			...
			for (int i = 0; i < MAX_ITEM; i++) {
				if (pc.item[i].useFlag && (str = strstr(pc.item[i].memo, "내구력")) && strstr(str, "회복")) {
					item_id = i;
					if (strstr(pc.item[i].name, "고기")) {
						meat = i;
						break;
					}
				}
			...
			// 주술
			if ((method > -1 && method < MAX_MAGIC) && magic[method].useFlag && (str = strstr(magic[method].memo, "내구력")) && strstr(str, "회복")) {
				if (pc.familySprite == 0) {
					if ((magic[method].mp * 80 / 100) <= p_party[BattleMyNo]->mp)
						magic_id = method;
				}
			...
			if (target > -1) {
				if (item_id > -1 || magic_id > -1) {
					if (meat > -1) {//使用肉补耐久力 // 고기 우선 사용
						done = 1;
						sprintf(moji, "I|%X|%X", meat, target);
						AI_Choosen = "고기를 사용하여 내구력을 회복하십시오.";
					}
					else {
						if (MAX_MAGIC > AI_method[0] && AI_method[0] >= 0) {//使用精灵补耐久力 // 주술 사용
							if (magic[AI_method[0]].target != MAGIC_TARGET_MYSELF || target == BattleMyNo) {
								if (magic_id > -1) {
									...
									AI_Choosen = "주술을 사용하여 내구력 회복";
									sprintf(moji, "J|%X|%X", magic_id, target);
								}
								else {
									StockChatBufferLine("회복 주술을 사용할 수 없습니다. 내구약을 사용합니다.", FONT_PAL_WHITE);
								}
							}
						}
						if (!done) {//使用道具补耐久力 // 아이템 사용
							if (item_id > -1) {
								done = 1;
								sprintf(moji, "I|%X|%X", item_id, target);
								AI_Choosen = "아이템을 사용하여 내구력 회복";
							}
							else {
								StockChatBufferLine("아이템이 부족하여 내구력으로 회복할 수 없습니다.", FONT_PAL_WHITE);
							}
						}
					}
				}
				else
					StockChatBufferLine("내구력을 회복할 수 없습니다.", FONT_PAL_WHITE);
			}
		}
		// 부활
		if (0 == done) {
			...
			if (target > -1) {//宠物死了
				if (MAX_MAGIC == AI_method[3]) {
					int i;
					for (i = 0; i < MAX_ITEM; i++) {
						// 아이템 설명에 필요한 글자 : "부활" // 예시)각성제(500),내구력 500 전후 회복하며 부활 // '부활'만 해두었더니 샴기르 목걸이 부활 글자 인식
						//if( pc.item[i].useFlag && strstr( pc.item[i].memo, "气绝回复")){
						if (pc.item[i].useFlag && strstr(pc.item[i].memo, "부활") && strstr(pc.item[i].name, "각성제")) {
							AI_Choosen = "부활 아이템 사용";
							done = 1;
					...
					if (i >= MAX_ITEM)
						StockChatBufferLine("부활 아이템이 없습니다.", FONT_PAL_WHITE);
				}
				else {
					int method = AI_method[3];
					// 주술 설명에 필요한 글자 : "기절상태" 뒤에 "회복" // 예시)부활의 정령 Lv1,기절상태 회복 후 내구력 회복 100 
					if (method > -1 && magic[method].useFlag && (str = strstr(magic[method].memo, "기절상태")) && strstr(str, "회복")) {
						if (pc.familySprite == 0) {
							if ((magic[method].mp * 80 / 100) <= p_party[BattleMyNo]->mp)
								done = 1;
						}
						...
						if (done == 1) {
							...
							StockChatBufferLine("기력이 부족하여 부활 주술을 사용할 수 없습니다.", FONT_PAL_WHITE);
					}
					else
						StockChatBufferLine("부활 대상이 설정되지 않았습니다.", FONT_PAL_WHITE);
				}
			}
		}
#ifndef _ATTACK_AI
		...
		if (0 == done && ((p_party[BattleMyNo]->maxMp * AI_limit[1] / 100) > p_party[BattleMyNo]->mp)) {//补气
...
#endif
			{
				int i;
				for (i = 0; i < MAX_ITEM; i++) {
					// 아이템 설명에 필요한 글자 : "기" 뒤에 "회복" // 예시)아부의 물,기력을 100 전후 회복
					if (pc.item[i].useFlag && (str = strstr(pc.item[i].memo, "기")) && strstr(str, "회복")) {
						AI_Choosen = "기력 회복 아이템을 사용합니다.";
						done = 1;
				...
				if (i >= MAX_ITEM) {
					...
					KeyboardReturn();
#endif
					StockChatBufferLine("기력 회복 아이템이 존재하지 않습니다.", FONT_PAL_WHITE);
				}
			}
		}
		if (0 == done) {
...
void AI_OtherProc() {
	...
	int x, y, i;
	static char szFunctionName[AI_OTHER_BTN][12] = { "","고기버리기" };
