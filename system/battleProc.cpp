	...
	if( BattleBpFlag & BATTLE_BP_ENEMY_SURPRISAL || BattleBpFlag & BATTLE_BP_PLAYER_SURPRISAL ){
		...
		if( pActSurprisalWnd->hp > 0 ){
			...
			if( BattleBpFlag & BATTLE_BP_ENEMY_SURPRISAL ){
				StockFontBuffer( pActSurprisalWnd->x + 38, pActSurprisalWnd->y + 40, FONT_PRIO_FRONT, 0, " 기습 공격을 당했다 ", 0 );
			}
			// ????????????
			if( BattleBpFlag & BATTLE_BP_PLAYER_SURPRISAL ){
				StockFontBuffer( pActSurprisalWnd->x + 38, pActSurprisalWnd->y + 40, FONT_PRIO_FRONT, 0, "기습 공격을 했다.", 0 );
			}
		}
	}
}
...
*/
		...
		case BATTLE_SUBPROC_CHAR_IN:			// ?
			...
			if( action_inf == 3 ){
				...
				//JL 2016.8.25 自动战斗
				void JL_自动战斗();
				extern int g_WgRecvHwnd;
				
				if (g_WgRecvHwnd)	//如果有窗口挂接了，每次战斗到这里交给外挂判断是否需要自动战斗，此时 SubProcNo = 3
				{
					JL_自动战斗();
				}
				else
		...
		case BATTLE_SUBPROC_CMD_INPUT:			// 4
#ifdef _STONDEBUG_
			if(offlineFlag == TRUE){
				SubProcNo++;
				break;
	...
	if( BackBufferDrawType != DRAW_BACK_PRODUCE ){ 
		...
		if( mouse.onceState & MOUSE_RIGHT_CRICK && BattleMyNo >= 20 ){
			...
			//RemoveAction(pActAudienceExitWnd);
			//pActAudienceExitWnd = NULL;
		...
		if( pActAudienceExitWnd != NULL ){
			...
			if( pActAudienceExitWnd->hp > 0 ){
				// ??
				StockFontBuffer( pActAudienceExitWnd->x + 30, pActAudienceExitWnd->y + 28, FONT_PRIO_FRONT, 0, 	"마우스 오른쪽 버튼 클릭", 0 );
				StockFontBuffer( pActAudienceExitWnd->x + 30, pActAudienceExitWnd->y + 52, FONT_PRIO_FRONT, 0, 	"시청종료", 0 );
			}
		}
	}
