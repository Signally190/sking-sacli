...
#ifdef _STONDEBUG_
				MessageBoxNew(hWnd, "SurfaceInfo:建立VideoRAM Surface失败！", "??", MB_OK | MB_ICONSTOP);
#endif
				vramFullFlag = TRUE;
			}
			else {
...
#ifdef _STONDEBUG_
				MessageBoxNew(hWnd, "建立SysRAM Surface失败！", "??", MB_OK | MB_ICONSTOP);
#endif
				return FALSE;
			}
...
#ifdef _STONDEBUG_
				MessageBoxNew(hWnd, "alpha记忆体配置失败！", "??", MB_OK | MB_ICONSTOP);
#endif
				return FALSE;
			}
			else {
...
#ifdef _STONDEBUG_
				MessageBoxNew(hWnd, "建立SysRAM Surface(2)失败！", "??", MB_OK | MB_ICONSTOP);
#endif
				return FALSE;
			}
			else SysramSurfaceCnt++;
