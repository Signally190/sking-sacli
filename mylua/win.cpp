...
bool LUA_WINDOW_OPEN = false;	//用于外挂[快速原登]时判断lua窗口是否打开了
#endif

static int InitWindow(lua_State *L) 
{
	LUA_WINDOW_OPEN = true;
	size_t l;
	...
	getStrSplit(windows.message.data[0], (char *)data, 1024, 25, msgWNLen);
	return 1;
}
