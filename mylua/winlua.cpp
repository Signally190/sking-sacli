...
lua_State* FindWinIntLua( int wintype, char *data )
{
	...
	if (lua == NULL){
		char filename[256];
#ifdef _LUA加密
		sprintf_s(filename, "%s//win//%d.sq", _LUA_PATCH_, wintype);
#else
...
static const luaL_Reg lualibs[] = {
	...
	{ "ChengHao",		luaopen_ChenghaoList },
	{ "GuangHuan",		luaopen_GuangHuanList },
	{ "PiFu",		luaopen_PiFuList },
...
#ifdef _LUA加密
VMProtectBegin("lua");
	TEA解密((long*)luabuff,(luamaxlen/4),(long *)VMProtectDecryptStringA(_LUA_KEY_));
VMProtectEnd();
#endif
...
int decryptLUA2(lua_State *L, char *filename){
	FILE *f;
	char loadfilename[256];
	int luamaxlen = 0;
	unsigned char *luabuff;
...
#else
	int re = decryptLUA2(salua->lua, filename);
	if(re!=0){
		char buf[100];
		wsprintfA(buf, "文件：%s 错误提示：%s 行数：%d\n", filename, (char *)lua_tostring(salua->lua, -1), re);
	//	printf("文件：%s 错误提示：%s 行数：%d\n",filename,(char *)lua_tostring(salua->lua, -1),re);
		lua_gc(salua->lua, LUA_GCCOLLECT, 0);
		return TRUE;	
	}
...
void LoadStoneAgeLUA(char *path){
	...
	do{
		...
		if (wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY){ 
			...
			extern int strcmptail(char *s1, char *s2);
#ifdef _LUA加密
			if( strcmptail( wfd.cFileName, ".sq" ) == 0)
#else
...
int FreeGetBattleMap(int floor)
{
	...
	if (lua == NULL){
		char filename[256];
#ifdef _LUA加密
		sprintf_s(filename, "%s//map//battlemap.sq",_LUA_PATCH_);
#else
...
char* FreeGetTitleStr(int id)
{
	...
	if (lua == NULL){
		char filename[256];
#ifdef _LUA加密
		sprintf_s(filename, "%s//title.sq",_LUA_PATCH_);
#else
